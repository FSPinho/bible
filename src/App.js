...
const backgroundJob = {
  ...
  job: async () => {
    ...
    if (existing) {
      ...
      const dailyRef = FireBase.firestore()
        .collection('daily')
        .where('schedule', '==', date)
        .limit(1);
...
export default class App extends Component {
  async componentDidMount() {
    ...
    const channelDaily = new FireBase.notifications.Android.Channel(
      'daily',
      'Oração diária',
      FireBase.notifications.Android.Importance.Max,
    ).setDescription('Canal de orações diárias');
    const channelArticle = new FireBase.notifications.Android.Channel(
      'article',
      'Histórias Reais',
      FireBase.notifications.Android.Importance.Max,
